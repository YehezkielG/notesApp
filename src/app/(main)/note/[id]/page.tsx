...
import { Heart, MessageCircle, Share2, Lock, Globe } from "lucide-react";
...
export default async function NoteDetailPage({
  ...
  return (
    <>
    <article className="rounded-xl border border-transparent bg-white/70 backdrop-blur transition-shadow hover:shadow-sm">
      ...
      <span className="mt-5 block text-xs text-gray-500">{createdAt}</span>
        ...
        {Array.isArray(note.emotion) && note.emotion.length > 0 ? (
          note.emotion.map((item: { label: string; score: number }) => {
            ...
            return (
                <div
                key={item.label}
                className="group py-1 px-2 rounded-lg"
                style={{ backgroundColor: bgColor + "10", border: `1px solid ${bgColor}33` }}
                >
